...
Chapter 4.3 책임 분리를 통한 애플리케이션 설계 문서

(작업 시간이 모자라서, 제대로된 문서가 없는점 양해 부탁드립니다.)

Q. 기존 트랜잭션에서 어떤 부분에 `SAGA Pattern`을 구현할까?

도메인 모듈간 결합이 되는 곳! -> `src/api/concert/usecase/concert.usecase.pay-seat.ts`

서버 아키텍처의 구성 요소를 모두 `모듈 단위`로 구성, 하나의 도메인 모듈은 기존 트랜잭션 방식을 사용. (DB 트랜잭션 공통화 O)

그러나, 모듈간 결합(잔액 차감 + 좌석 결제 + 영수증 생성)은 DB 트랜잭션을 노출하면, 아키텍처가 너무 지저분해짐.

이때, Kafka를 이용하여 SAGA Pattern을 구현 한다.
...
- `userId = ''` 로 검색 ✅
    - userId는 1:1 관계를 유지하기 위해, unique index를 사용
    - postgres에서는 unique index 사용 시 b-tree 형태로 인덱싱
...
- `seatNo`로 검색 🔴
    - Int 타입
    - seatNo는 좌석이 예약(=생성)되었는지 확인하는 용도
    - 순차적으로 정렬할 필요가 없기 때문에 `Hash` 타입으로 생성
